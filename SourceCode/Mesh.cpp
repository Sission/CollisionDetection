...
Mesh::Mesh(const wchar_t* full_path): Resource(full_path)
{
	...
	for (size_t s = 0; s < shapes.size(); s++)
	{
		...
		list_vertices_mem.reserve(shapes[s].mesh.indices.size());
		list_indices_mem.reserve(shapes[s].mesh.indices.size());
		...
		for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++)
		{
			...
			for (unsigned char v = 0; v < num_face_verts; v++)
			{
				...
				VertexMesh vertex_mem(Vector3D(vx, vy, vz), Vector2D(tx, ty), Vector3D(nx, ny, nz));

				list_vertices.push_back(vertex);
				list_vertices_mem.push_back(vertex_mem);

				list_indices.push_back((unsigned int)index_offset + v);
				list_indices_mem.push_back((unsigned int)index_offset + v);
			}
...
std::vector<VertexMesh> * Mesh::getVertexMem()
{
	// TODO: insert return statement here
	return &list_vertices_mem;
}

std::vector<unsigned int> * Mesh::getIndexMem()
{
	// TODO: insert return statement here
	return &list_indices_mem;
}

void Mesh::readFromVertexBuffer()
{
